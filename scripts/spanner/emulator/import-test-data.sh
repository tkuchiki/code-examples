...
usage() {
    ...
    cat <<EOC
Usage: ${_script} [-h] [-c GCLOUD_CONFIG] [-p GCP_PROJECT] [-i SPANNER_INSTANCE] [-d SPANNER_DATABASE] [-r ROWS]

Import test data to Spanner emulator table using spanner-cli
...
while getopts p:i:d:c:r:h OPT; do
    case $OPT in
...
while :; do
    ...
    } | sed -e "s/,$//" | spanner-cli -p ${gcp_project} -i ${spanner_instance} -d ${spanner_database}
    
    l=$(( ${l} + ${n} ))
    echo "Imported ${l}/${rows} rows"
